...
"    char *: margo_print_span_from_cstr \\\n",
        ...
        "    uint64_t magic;\n",
        "    size_t size;\n",
        "} margo_fallback_alloc_header_t;\n",
        "#define MARGO_FALLBACK_ALLOC_MAGIC UINT64_C(0x4D4152474F4D454D)\n",
        "static inline void *margo_fallback_alloc(size_t bytes) {\n",
        "    if (bytes == 0) {\n",
        "        return NULL;\n",
        "    }\n",
        "    size_t total = sizeof(margo_fallback_alloc_header_t) + bytes;\n",
        "    margo_fallback_alloc_header_t *hdr = (margo_fallback_alloc_header_t *)malloc(total);\n",
        "    if (!hdr) {\n",
        "        return NULL;\n",
        "    }\n",
        "    hdr->magic = MARGO_FALLBACK_ALLOC_MAGIC;\n",
        "    hdr->size = bytes;\n",
        "    return (void *)(hdr + 1);\n",
        "}\n",
        "static inline bool margo_is_fallback_alloc(const void *ptr) {\n",
        "    if (!ptr) {\n",
        "        return false;\n",
        "    }\n",
        "    const margo_fallback_alloc_header_t *hdr = ((const margo_fallback_alloc_header_t *)ptr) - 1;\n",
        "    return hdr->magic == MARGO_FALLBACK_ALLOC_MAGIC;\n",
        "}\n",
        "static inline void margo_builtin_free_ptr(void *ptr) {\n",
        "    if (!ptr) {\n",
        "        return;\n",
        "    }\n",
        "    if (margo_is_fallback_alloc(ptr)) {\n",
        "        margo_fallback_alloc_header_t *hdr = ((margo_fallback_alloc_header_t *)ptr) - 1;\n",
        "        hdr->magic = 0;\n",
        "        free(hdr);\n",
        "        return;\n",
        "    }\n",
